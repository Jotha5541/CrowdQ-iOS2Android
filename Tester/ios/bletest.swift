...
public struct CrowdQExchange {
    ...
    public func pack() -> String {
        ...
        let bytes = byteArray(from: block1) + byteArray(from: Int32(argument))
...
class BluetoothAdvertiser : NSObject, CBPeripheralManagerDelegate {
    ...
    func peripheralManagerDidUpdateState(_ peripheral: CBPeripheralManager) {
        ...
        case .poweredOn:
            ...
            let uuid = CBUUID(string: "EEEE")
            let packet = CrowdQExchange(sequence:sequence,tag:.load,argument:0,payload:"free/demo")
            sequence += 1
    ...
    func peripheralManagerDidStartAdvertising(_ peripheral: CBPeripheralManager, error: Error?) {
        ...
        if let error = error {
            ...
            print("🚀 Successfully advertising EEEE!")
        }
    }

    var timer: Timer?
    var commands : [CrowdQExchange] = []
    func startTimer() {
        timer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] timer in
            guard let self = self, self.commands.count > 0 else { return }
            let uuid = CBUUID(string: "EEEE")
            let curr = self.commands[self.sequence % self.commands.count]
            let packet = CrowdQExchange(sequence:self.sequence,tag:curr.tag,argument:curr.argument,payload:curr.payload)
            print(packet)
            self.sequence += 1
            let advertisementData: [String: Any] = [
                CBAdvertisementDataServiceUUIDsKey: [uuid],
...
control.commands = [
  CrowdQExchange(sequence:0,tag:.command,argument:3,payload:".left"),
  CrowdQExchange(sequence:0,tag:.command,argument:3,payload:".right"),
  CrowdQExchange(sequence:0,tag:.command,argument:3,payload:".left"),
  CrowdQExchange(sequence:0,tag:.command,argument:3,payload:".right"),
  CrowdQExchange(sequence:0,tag:.command,argument:3,payload:".left"),
  CrowdQExchange(sequence:0,tag:.command,argument:3,payload:".right"),
